...
function drawMarkers(){
  ...
  items.forEach((item) => {
    const marker = L.marker([item.lat,item.lng]).addTo(map).bindPopup(`<b>${item.title}</b><br>${item.address || '주소 정보 없음'}`)
    markers.push(marker)
  })
  if(items.length){
...
<section class="page"><div class="container">
  <div class="page-head"><div><div class="eyebrow">TOUR MAP</div><h1>부산 관광 지도</h1><p>TourAPI JSON 좌표를 이용해 카테고리별 POI를 표시합니다.</p></div></div>
  <div class="map-layout">
    <aside class="panel category-list"><button class="category-btn" :class="{active:selectedCategory===category}" v-for="category in categories" @click="selectedCategory=category"><span>{{ icon(category) }} {{ category }}</span><b>{{ formatNumber(totals[category]) }}</b></button></aside>
    <div><div class="panel map-wrap"><div class="map-toolbar"><strong>{{ selectedCategory }} POI</strong><span>최대 180개 마커 표시</span></div><div ref="mapElement" class="map"></div></div>
      <div class="poi-list"><article class="panel poi-card" v-for="item in visibleItems" @click="focus(item)"><img v-if="item.image" :src="item.image" @error="$event.target.style.display='none'"><h4>{{ item.title }}</h4><p>{{ item.address||'주소 정보 없음' }}</p></article></div>
    </div>
  </div>
</div></section>
</template>
